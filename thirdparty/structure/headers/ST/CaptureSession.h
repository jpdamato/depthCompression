...
struct ST_API CaptureSessionDelegate
{
    ...
    */
    ...
    /** @brief Gets called when the state of dynamic calibration changes. @see DynamicCalibrationState
        @param session The parent CaptureSession this delegate is attached to.
        @param status The status identifier.
    */
    virtual void dynCalibrationEventDidOccur(CaptureSession* session, DynamicCalibrationState status) {}
};
